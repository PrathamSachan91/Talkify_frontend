      ...
      {segments.map((seg, i) => {
        ...
        return (
          <circle
            ...
            strokeDashoffset={-circ + offset + circ * 0.25}
            strokeLinecap="round"
            style={{
...
const AdminDashboard = () => {
  ...
  const {
    ...
    isLoading: lg,
    refetch: rg,
  } = useQuery({ queryKey: ["allGroups"], queryFn: fetchAllGroups });
  const {
    data: conversations = [],
    isLoading: lc,
    refetch: rc,
    ...
    queryFn: fetchAllConversation,
  });

  const isLoading = lu || lg || lc;

  // Normalise user_status to lowercase at source — DB stores "Active"/"Banned"
  const users = useMemo(
    () =>
      rawUsers
        ...
        .map((u) => ({
          ...u,
          status: (u.user_status || "active").toLowerCase(),
        })),
    [rawUsers, hiddenUsers],
  );

  const groups = useMemo(
    () => rawGroups.filter((g) => !hiddenGroups.has(g.conversation_id)),
    [rawGroups, hiddenGroups],
  );
  ...
  const { convById, privateConvByUser } = useMemo(() => {
    ...
    conversations.forEach((c) => {
      ...
      if (c.type === "private") {
        // Map both participants so lookup works regardless of who is user1/user2
        const other =
          c.user1_id === currentUser?.auth_id ? c.user2_id : c.user1_id;
        if (other) privateConvByUser.set(other, c);
      }
    });
  ...
  const handleToggleBan = async (user) => {
    // DB stores "Active"/"Banned" (capitalised) — send that format to API
    const newStatusDB = user.status === "banned" ? "Active" : "Banned";
    // Optimistic update — flip immediately so UI responds without waiting for server
    queryClient.setQueryData(["users"], (old = []) =>
      old.map((u) =>
        u.auth_id === user.auth_id ? { ...u, user_status: newStatusDB } : u,
      ),
    );
    try {
      await handelStatus(user.auth_id, newStatusDB);
    } catch (e) {
      // Revert on failure by re-fetching real data
      ru();
    }
  };
  const toggleSelect = (authId) =>
    setSelectedIds((prev) => {
      const n = new Set(prev);
      n.has(authId) ? n.delete(authId) : n.add(authId);
      return n;
    });
  const hideUser = (authId) => {
    setHiddenUsers((prev) => new Set([...prev, authId]));
    setSelectedIds((prev) => {
      const n = new Set(prev);
      n.delete(authId);
      return n;
  ...
  const hideUsers = (ids) => {
    setHiddenUsers((prev) => new Set([...prev, ...ids]));
  ...
  const confirmDelete = (title, body, onConfirm) =>
  ...
  const statCards = [
    {
      ...
      spark: [
        0,
        Math.round(users.length * 0.3),
        Math.round(users.length * 0.55),
        Math.round(users.length * 0.75),
        Math.round(users.length * 0.9),
        users.length,
      ],
    },
    {
      label: "Banned",
      value: bannedCount,
      sub: `${users.length - bannedCount} normal`,
      icon: <Ban size={17} />,
      color: "#f87171",
      spark: [
        0,
        Math.round(bannedCount * 0.2),
        Math.round(bannedCount * 0.5),
        bannedCount,
      ],
    },
    {
      label: "Total Groups",
      value: groups.length,
      sub: `${groupConvs.length} conversations`,
      icon: <Hash size={17} />,
      color: "#f59e0b",
      spark: [
        0,
        Math.round(groups.length * 0.4),
        Math.round(groups.length * 0.7),
        groups.length,
      ],
    },
    {
      ...
      value: totalUnread,
      sub: `${privateConvs.length} DMs active`,
      icon: <MessageSquare size={17} />,
      color: "#818cf8",
      spark: [
        0,
        Math.round(totalUnread * 0.3),
        Math.round(totalUnread * 0.6),
        totalUnread,
      ],
    },
  ];
    ...
    >
      ...
      >
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div
              style={{
                ...
                borderRadius: 9,
                background:
                  "linear-gradient(135deg, var(--accent-primary, #2dd4bf) 0%, var(--accent-secondary, #22c55e) 100%)",
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
              }}
            ...
            return (
              <button
                ...
                style={{
                  ...
                  border: "none",
                  background: active
                    ? "linear-gradient(135deg, rgba(45,212,191,0.14) 0%, rgba(34,197,94,0.08) 100%)"
                    : "transparent",
                  color: active
                    ? "var(--accent-primary, #2dd4bf)"
                  ...
                  borderLeft: `2px solid ${active ? "var(--accent-primary, #2dd4bf)" : "transparent"}`,
                }}
              >
                {item.icon}
        ...
        <div
          style={{
            padding: "14px 10px",
            borderTop: "1px solid var(--border-main, rgba(255,255,255,0.06))",
            display: "flex",
            flexDirection: "column",
            gap: 6,
          }}
      ...
      >
        ...
        {activeTab === "overview" && (
          <div style={{ animation: "tabIn 0.2s ease-out" }}>
            ...
            >
              ...
              >
                <h3
                  style={{ margin: "0 0 4px", fontSize: 14, fontWeight: 700 }}
                >
                  Platform Growth
                </h3>
                <p
                ...
                >
                  Cumulative users · groups · conversations
                </p>
                {isLoading ? (
                  <Skel h={130} />
                ) : (
                    ...
                    datasets={[
                      {
                        ...
                        data: [
                          0,
                          Math.round(users.length * 0.15),
                          Math.round(users.length * 0.32),
                          Math.round(users.length * 0.5),
                          Math.round(users.length * 0.65),
                          Math.round(users.length * 0.78),
                          Math.round(users.length * 0.88),
                          users.length,
                        ],
                      },
                      {
                        ...
                        data: [
                          0,
                          Math.round(groups.length * 0.2),
                          Math.round(groups.length * 0.4),
                          Math.round(groups.length * 0.58),
                          Math.round(groups.length * 0.72),
                          Math.round(groups.length * 0.84),
                          groups.length,
                          groups.length,
                        ],
                      },
                      {
                        ...
                        data: [
                          0,
                          Math.round(conversations.length * 0.18),
                          Math.round(conversations.length * 0.36),
                          Math.round(conversations.length * 0.54),
                          Math.round(conversations.length * 0.7),
                          Math.round(conversations.length * 0.84),
                          Math.round(conversations.length * 0.93),
              ...
              >
                ...
                {isLoading ? (
                  ...
                  >
                    <Donut
                      segments={[
                        { label: "Active", value: activeCount || 1 },
                        { label: "Banned", value: bannedCount },
                        {
                          label: "Online",
                          value: users.filter((u) => online.has(u.auth_id))
                            .length,
                        },
                      ]}
                    />
                  ...
                  {sortedConvs.slice(0, 5).map((conv) => (
                    <div
                      ...
                      style={{
                        ...
                        border: `1px solid ${conv.message_count > 0 ? "rgba(45,212,191,0.12)" : "var(--border-main)"}`,
                        cursor: "pointer",
                      }}
                    ...
                    >
                      ...
                      >
                        {conv.type === "group" ? (
                          filteredGroups[0]?.group_image ? (
                            <img
                              src={filteredGroups[0].group_image}
                              alt=""
                              style={{
                                width: "100%",
                                height: "100%",
                        ...
                        ) : conv.user1?.profile_image ? (
                          <img
                            src={conv.user1.profile_image}
                            alt=""
                            style={{
                              width: "100%",
                              height: "100%",
                          ...
                          conv.user1?.user_name?.charAt(0).toUpperCase() || "U"
                        )}
                      </div>
                      ...
                      <div style={{ flex: 1, overflow: "hidden" }}>
                        ...
                        >
                          <span style={{ fontSize: 12, fontWeight: 600 }}>
                            {conv.type === "group"
                              ? conv.group_name ||
                                `Group #${conv.conversation_id}`
                              : [conv.user1?.user_name, conv.user2?.user_name]
                                  .filter(Boolean)
                                  .join(" ↔ ") || `DM #${conv.conversation_id}`}
                          </span>
                          <span
                            style={{
                              fontSize: 10,
          ...
          <div style={{ animation: "tabIn 0.2s ease-out" }}>
            ...
            >
              <div>
                ...
                >
                  {users.length} total · {activeCount} active · {bannedCount}{" "}
                  banned · {users.filter((u) => online.has(u.auth_id)).length}{" "}
                  online
                </p>
              </div>
              {selectedIds.size > 0 && (
                <button
                  onClick={() =>
                    confirmDelete(
                      ...
                      `Remove ${selectedIds.size} user(s) from this view.`,
                      () => {
                        hideUsers(selectedIds);
                    ...
                    pagedUsers.map((user, i) => {
                      ...
                      return (
                          ...
                          style={{
                            ...
                            borderBottom:
                              i < pagedUsers.length - 1
                                ...
                                : "none",
                            background: selectedIds.has(user.auth_id)
                              ? "rgba(45,212,191,0.04)"
                              : "transparent",
                        ...
                        >
                          ...
                          <input
                            type="checkbox"
                            checked={selectedIds.has(user.auth_id)}
                            onChange={() => toggleSelect(user.auth_id)}
                            style={{
                          ...
                          >
                            ...
                            <span
                              style={{
                                fontSize: 11,
                                color: isOn ? "#34d399" : "var(--text-muted)",
                              }}
                            >
                              {isOn ? "On" : "Off"}
                            </span>
                          ...
                          <div style={{ display: "flex", gap: 5 }}>
                            ...
                            <button
                              onClick={() =>
                                confirmDelete(
                                  "Remove user?",
                                  `Remove "${user.user_name}" from the admin view?`,
                                  () => {
                                    hideUser(user.auth_id);
                                    setConfirmModal(null);
                                  },
                                )
                              }
        ...
        {activeTab === "groups" && (
          <div style={{ animation: "tabIn 0.2s ease-out" }}>
            ...
            <div
              style={{ position: "relative", marginBottom: 14, maxWidth: 300 }}
            >
              <Search
                size={13}
                style={{
                      ...
                      >
                        ...
                        <button
                          onClick={() =>
                            confirmDelete(
                              "Delete group?",
                              `Permanently delete "${group.group_name}"? This cannot be undone.`,
                              () => {
                                deleteGroup(group.conversation_id); //del
                                setConfirmModal(null);
                              },
                            )
                          }
          ...
          <div style={{ animation: "tabIn 0.2s ease-out" }}>
            ...
            >
              <div>
                ...
                >
                  ...
                  conversations · {totalUnread} total messages
                </p>
              </div>
              <div
                style={{
            ...
            >
              {[
                ...
                {
                  label: "Active Convs",
                  value: conversations.filter((c) => c.message_count > 0)
                    .length,
                  sub: "have messages",
                  color: "#f87171",
                ...
                {
                  label: "Group Chats",
                  value: groupConvs.length,
                  sub: `${privateConvs.length} private`,
                  color: "#f59e0b",
                  icon: <Hash size={15} />,
                },
              ].map((s, i) => (
              ...
              <div style={scrollWrap}>
                <div style={{ minWidth: 640 }}>
                  <div
                    style={{
                      display: "grid",
                      gridTemplateColumns: "48px 180px 1fr 72px 76px 52px 44px",
                      padding: "10px 18px",
                      borderBottom: "1px solid var(--border-main)",
                      fontSize: 10,
                      fontWeight: 700,
                    ...
                    sortedConvs.map((conv, i) => (
                      <div
                        ...
                        style={{
                          display: "grid",
                          gridTemplateColumns:
                            "48px 180px 1fr 72px 76px 52px 44px",
                          padding: "10px 18px",
                          alignItems: "center",
                          borderBottom:
                            i < sortedConvs.length - 1
                              ? "1px solid var(--border-main)"
                              : "none",
                          background:
                            conv.message_count > 0
                      ...
                      >
                        ...
                        {/* Name: group name or "User1 ↔ User2" for DMs */}
                        <div style={{ overflow: "hidden" }}>
                          <p
                            style={{
                              margin: 0,
                        ...
                        <div style={{ overflow: "hidden" }}>
                          ...
                          >
                            {conv.last_message || (
                              <span style={{ opacity: 0.35 }}>No messages</span>
                            )}
                          </p>
                        </div>
                        <span
